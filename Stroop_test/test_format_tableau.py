...
def recodage(data_participant, name, data_global):
	...
	data_participant['Réponses'] = data_participant['Réponses'].replace({'b': "Bleu", 'r': "Rouge", 'v': "Vert"})
	...
	data_global['Réponse'] = data_global['Réponse'].replace({'b': "Bleu", 'r': "Rouge", 'v': "Vert"})
...
def score(data_participant, data_global):
	...
	Liste_rep_glob = data_global["Réponse"]

	# Résultats individuels
	Liste_mot_part = data_participant["Liste de mots"]
	Liste_rep_part = data_participant["Réponses"]
	...
	mean_groupe = data_global["Temps de réponse"].mean()
	# Temps moyen de réponse du participant
	mean_parti = data_participant["Temps de réponse"].mean()
